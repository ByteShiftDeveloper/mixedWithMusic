...
class SoundCloudActionCell: UICollectionViewCell {
    var user: User? {
        didSet {
            configure()
        }
    ...
    private var optionImage: UIImageView = {
     ...
     let titleLabel: UITextField = {
       let label = UITextField()
        ...
        label.placeholder = "Enter your SoundCloud Account URL"
    ...
    func configure() {
        if user?.igInfo != "" {
        titleLabel.text = user?.soundcloudInfo
        optionImage.image = UIImage(named: "iconfinder_soundcloud_1218163")
        } else{
            titleLabel.text = nil
        }
    }
