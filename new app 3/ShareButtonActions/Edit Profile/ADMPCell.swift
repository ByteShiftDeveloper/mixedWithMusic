...
protocol ADMPCellDelegate: class {
    func pickerSelect(_ cell: ADMPCell)
}

class ADMPCell: UITableViewCell {
    var pickerType : PickerType = .Artist
    var user: User? {
        didSet { configureUI() }
    }
    
    weak var delegate: ADMPCellDelegate?
...
//    titleLabel.widthAnchor.constraint(equalToConstant: 100).isActive = true
    ...
    self.contentView.addSubview(ADMPLabel)
    ADMPLabel.anchor(right: editButton.leftAnchor, paddingRight: 8)
    ADMPLabel.centerY(inView: self.contentView)
    ...
    func configureUI() {
        if pickerType == PickerType.Artist{
            titleLabel.text = "What do you consider youself?"
            ADMPLabel.text = user?.artistBand
        }
        else if pickerType == PickerType.City{
