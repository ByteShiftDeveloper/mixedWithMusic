...
//  new app 3
//
//  Created by Danyal on 15/10/2020.
...
class NewProfileVC : UIViewController, UIScrollViewDelegate, TPDataSource, TPProgressDelegate {
    
    var headerVC: HeaderView2?
...
//        view.showsHorizontalScrollIndicator = false
    ...
    func checkUserIsFollowed() {
        Service.shared.checkIfUserIsFollowed(uid: user!.uid) { isFollowed in
            ...
            if isFollowed == true {
                self.headerVC?.btnFollow.backgroundColor = UIColor(named: "FollowColor")
                self.headerVC?.btnFollow.setTitleColor(.white, for: .normal)
                self.headerVC?.btnFollow.setTitle("Following", for: .normal)
                
            } else if isFollowed == false {
                self.headerVC?.btnFollow.backgroundColor = UIColor(named: "DefaultBackgroundColor")
                self.headerVC?.btnFollow.setTitleColor(UIColor(named: "BlackColor"), for: .normal)
                self.headerVC?.btnFollow.layer.borderColor = UIColor(named: "BlackColor")?.cgColor
