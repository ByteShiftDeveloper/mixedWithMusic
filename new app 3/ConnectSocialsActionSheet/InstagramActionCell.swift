...
class InstagramActionCell: UICollectionViewCell {
    var user: User? {
        didSet {
            configure()
        }
    ...
    private var optionImage: UIImageView = {
     ...
     let titleLabel: UITextField = {
       let label = UITextField()
        ...
        label.placeholder = "Enter your Instagram Account URL"
    ...
    func configure() {
        if user?.igInfo != "" {
        titleLabel.text = user?.igInfo
        optionImage.image = UIImage(named: "logo-instagram-png-13560")
        } else{
            titleLabel.text = nil
        }
    }
