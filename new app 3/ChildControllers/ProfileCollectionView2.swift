                ...
                print("audio is \(snapshot)")
                    ...
                    guard let dictionary = snapshot.value as? [String: AnyObject] else { return }
                                ...
                                UserService.shared.fetchUser(uid: uid) { user in
                                    ...
                                    let song = SongPost(id: snapshot.key, author: user, title: trackTitle, coverImage: imageURL, audioUrl: audioUrls , audioName: audioNames , timestamp: timestamp)
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if selectedFilter == .uploads {
            if indexPath.section == 2 {
                let controller = SelectSongController(collectionViewLayout: UICollectionViewFlowLayout())
