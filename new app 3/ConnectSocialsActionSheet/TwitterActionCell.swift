...
class TwitterActionCell: UICollectionViewCell {
    var user: User? {
        didSet {
            configure()
        }
    ...
    let titleLabel: UITextField = {
       let label = UITextField()
        ...
        label.placeholder = "Enter your Twitter Account URL"
    ...
    func configure() {
        if user?.igInfo != "" {
        titleLabel.text = user?.twitterInfo
        optionImage.image = UIImage(named: "PinClipart.com_throne-clipart-black-and_906696")
        } else{
            titleLabel.text = nil
        }
    }
